...
#else 
	template	<typename K, class V, class _Traits = stdext::hash_compare<Key, less<Key> >, typename allocator = xalloc<std::pair<K,V> > >	
	class	xr_hash_map	: public std::unordered_map<K, V, std::hash<V>, _Traits, allocator> //stdext::hash_map<K,V,_Traits,allocator>	
	{ 
	public: 
		u32 size() const 
		{
